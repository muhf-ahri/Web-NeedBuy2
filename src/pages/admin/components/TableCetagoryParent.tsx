...
const TableCategoryParent: React.FC<TableCategoryParentProps> = ({
  data,
  page,
  totalPages,
  totalItems,
        ...
        <table className="stack-table w-full text-sm">
          <thead>
            <tr className="border-b border-[#f2f4f6] text-[11px] font-semibold uppercase text-[#737686]">
              <th className="pb-2 pr-2 text-left">Nama Kategori</th>
              <th className="pb-2 pr-2 text-left">Deskripsi</th>
              <th className="pb-2 pr-2 text-left">Total Produk</th>
              <th className="pb-2 pr-2 text-left">Subkategori</th>
            ...
            {data.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-10 text-center text-[#737686]">
                  Belum ada kategori induk. Klik "Tambah Induk" untuk memulai.
                </td>
              </tr>
            ) : (
              data.map((category) => (
                <tr key={category.id} className="text-[13px] transition-colors hover:bg-[#f5f7fb]">
                  ...
                  <td className="py-2.5 pr-2 text-[#434655]">
                    {category._count.children.toLocaleString('id-ID')}
                  </td>
